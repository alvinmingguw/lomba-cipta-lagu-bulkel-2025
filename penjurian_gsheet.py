...
if nav == "📝 Penilaian":
    ...
    if audio_src and audio_src.get("mode"):
        ok = False
        # 1) GDrive by ID → stream URL
        if audio_src.get("id"):
            url = _gdrive_stream_url(audio_src["id"])
            if url:
                fmt = pick_audio_format(audio_src.get("mime"), url)
                st.audio(url, format=fmt)
                ok = True
            # fallback: unduh bytes bila perlu (lebih berat)
            if not ok:
                audio_bytes = drive_download_bytes(audio_src["id"])
                if audio_bytes:
                    fmt = pick_audio_format(audio_src.get("mime"))
                    st.audio(audio_bytes, format=fmt)
                    ok = True

        # 2) Direct URL yang sudah disediakan
        if (not ok) and audio_src.get("direct"):
            fmt = pick_audio_format(audio_src.get("mime"), audio_src.get("direct"))
            st.audio(audio_src["direct"], format=fmt)
            ok = True

        # 3) Local path
        if (not ok) and audio_src.get("mode") == "local" and audio_src.get("path") and os.path.exists(audio_src["path"]):
            try:
                with open(audio_src["path"], "rb") as f:
                    data = f.read()
                st.audio(data, format=pick_audio_format(None, audio_src["path"]))
                ok = True
            except Exception:
                pass

        if not ok:
            st.warning("Audio tidak bisa diputar. Pastikan file Drive diset **Anyone with the link (Viewer)** atau pindahkan ke storage publik (Supabase/GitHub Pages/Vercel).")
    else:
